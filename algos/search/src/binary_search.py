...
def binary_search_recursive(
    ...
    if len(sequence) == 1:
        ...
        if target == sequence[mid_idx]:
            ...
            return (mid_idx + recursive_call_result if recursive_call_result != -1
                    else recursive_call_result)
        else:
            return binary_search_recursive(target, sequence[:mid_idx])


def binary_search_recursive_two(
    target: TNum,
    sequence: Sequence[TNum],
) -> int:
    def binary_search_recursive_two_inner(
        target: TNum,
        sequence: Sequence[TNum],
        low: int,
        high: int,
    ) -> int:
        """Binary search algorithm - better recursive implementation w/o list slicing.
        ...
        Args:
            target: an element to look for.
            sequence: sequence of sorted data to look for element.
            low: first sequence index to look for target.
            high: last sequence index to look for target (inclusive).
